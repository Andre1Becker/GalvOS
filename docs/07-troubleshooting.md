...
**Warning:** changing this value rescales every rate-dependent optimizer parameter's *effective* value at once — density by `1/r`, the velocity ceiling by `r`, the acceleration ceiling by `r²`. Simply editing `galvo_rated_kpps` therefore shifts how every existing profile behaves, sometimes drastically (a factor-of-3 correction, for instance, is a ×3 density swing and a ×9 acceleration-ceiling swing). If you need to correct a wrong rated value **without** invalidating existing tuning, transform the profiles' stored raw values by `k^e` (`k = rated_old / rated_new`, `e` per parameter as listed in [Chapter 5 — PPS Scaling](05-optimizer.md#pps-scaling)) instead of editing the field and re-tuning from scratch — the output rate cancels out of `k`, so this is an exact change of units, not a re-tune. See `docs/migrations/README.md` for a worked derivation. Note also that a community preset's `optimizer_profile` block ships raw values with no rated-kpps anchor of its own (see [Chapter 9](09-contributing.md#contributing-a-community-preset)) — it is interpreted against *your* device's rated value, so a mismatch there silently distorts every imported preset the same way.

### Output rate is far above the galvo's rated kpps

**Symptom:** the galvo is rated 15 kpps but `galvo_kpps` reads 30–45.

**This is not necessarily wrong.** The rated figure is a mechanical large-signal spec (ILDA test pattern at `ilda_test_angle`); `galvo_kpps` is the DAC sample clock. Running the sample clock above the rated point rate is oversampling — the mirror low-passes what it cannot follow, and the extra samples improve dwell granularity and interpolation smoothness. See the `kpps` entry in the [glossary](11-glossary.md). The Projection tab's Angular Configuration card shows an informational (non-warning) line stating the mechanical tracking rate at the current exit angle for reference — it is not a limit on the sample clock.

**What to check instead:** whether `galvo_rated_kpps` still holds the datasheet value — check `GET /api/projection`. If it has drifted from the datasheet figure, the PPS-scaling ratio `r = rated / output` is off, and the optimizer is silently running with the wrong effective density/velocity/acceleration ceilings for every profile.

**Note on the autotune button:** it measures the *controller's* sample-output throughput — how fast the ESP32 can actually deliver commanded points before it starts falling behind — not the galvo's mechanical rating; the two are unrelated quantities, and the ceiling it returns has no relationship to `galvo_rated_kpps`. It searches the full 12–60 kpps range and reports `ceiling_not_reached` if the whole range comes back clean. Start a real pattern (`master_dimmer` > 0) before running it — an idle/blanked output never overflows at any rate, so the search would report a false clean ceiling. `points_per_sec` (from `/api/state`) vs. `/api/projection`'s `kpps` is the one-line health check for whether the board is actually keeping up at the currently configured rate.
